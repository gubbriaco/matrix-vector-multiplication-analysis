...
/**
 * Sparse Matrix Multiplication Loop 2 Pipeline, Unrolling Factor=2 and Partitioning Cyclic=4 (All 3) Design.
 ...
 */
void smvm_loop2PU2C4all3(int iFirstEl[rows+1], int iNonZeroEl[noZeroEl], DTYPE values[noZeroEl], DTYPE mulRes[size], DTYPE vector[size]) {
	loop1: for (int i=0; i<rows; i++) {
		DTYPE ytmp = 0;
		loop2: for (int k=iFirstEl[i]; k<iFirstEl[i+1]; k++) {
			#pragma HLS pipeline
			#pragma HLS unroll factor=2
			#pragma HLS array_partition variable=iNonZeroEl cyclic factor=4
			#pragma HLS array_partition variable=values cyclic factor=4
			#pragma HLS array_partition variable=vector cyclic factor=4
